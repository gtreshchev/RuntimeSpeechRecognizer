...
//~ RuntimeSpeechRecognizer Start
namespace VulkanInitFunctions
{
    ...
    void InitializeVulkanGetPhysicalDeviceFeatures2(vk::Instance& instance)
    {
    ...
    void InitializeVulkanCmdCopyBuffer(vk::Instance& instance)
    {
        if (!vkCmdCopyBuffer)
        {
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        //~ RuntimeSpeechRecognizer Start
        VulkanInitFunctions::InitializeVulkanGetPhysicalDeviceFeatures2(vk_instance.instance);
        VulkanInitFunctions::vkGetPhysicalDeviceFeatures2(device->physical_device, &device_features2);
...
static void ggml_vk_print_gpu_info(size_t idx) {
    ...
    //~ RuntimeSpeechRecognizer Start
    VulkanInitFunctions::InitializeVulkanGetPhysicalDeviceFeatures2(vk_instance.instance);
    VulkanInitFunctions::vkGetPhysicalDeviceFeatures2(physical_device, &device_features2);
...
void ggml_vk_instance_init() {
    ...
    //~ RuntimeSpeechRecognizer Start
    VulkanInitFunctions::InitializeVulkan();
    //~ RuntimeSpeechRecognizer End
    ...
    if (devices_env != nullptr) {
        ...
        while (ss >> tmp) {
            if(tmp >= num_available_devices) {
                ...
                GGML_ABORT("Invalid Vulkan device index");
                //~ RuntimeSpeechRecognizer End
            }
...
static void ggml_vk_buffer_write_nc_async(ggml_backend_vk_context * ctx, vk_context& subctx, vk_buffer& dst, size_t offset, const ggml_tensor * tensor, bool sync_staging = false) {
    ...
    //~ RuntimeSpeechRecognizer Start
    VulkanInitFunctions::InitializeVulkanCmdCopyBuffer(vk_instance.instance);
    VulkanInitFunctions::vkCmdCopyBuffer(subctx->s->buffer, staging->buffer, dst->buffer, 1, &buf_copy);
...
static void ggml_vk_buffer_write_2d_async(vk_context subctx, vk_buffer& dst, size_t offset, const void * src, size_t spitch, size_t width, size_t height, bool sync_staging = false) {
    ...
    //~ RuntimeSpeechRecognizer Start
    VulkanInitFunctions::InitializeVulkanCmdCopyBuffer(vk_instance.instance);
    VulkanInitFunctions::vkCmdCopyBuffer(subctx->s->buffer, staging_buffer->buffer, dst->buffer, 1, &buf_copy);
...
static void ggml_vk_buffer_copy_async(vk_context& ctx, vk_buffer& dst, size_t dst_offset, vk_buffer& src, size_t src_offset, size_t size) {
    ...
    //~ RuntimeSpeechRecognizer Start
    VulkanInitFunctions::InitializeVulkanCmdCopyBuffer(vk_instance.instance);
    VulkanInitFunctions::vkCmdCopyBuffer(ctx->s->buffer, src->buffer, dst->buffer, 1, &bc);
...
static void ggml_vk_add(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool dryrun = false) {
    ...
    ggml_vk_op_f32<vk_op_binary_push_constants>(ctx, subctx, src0, src1, nullptr, dst, GGML_TYPE_F16, {
        (uint32_t)ggml_nelements(src0),
        (uint32_t)src0->ne[0], (uint32_t)src0->ne[1], (uint32_t)src0->ne[2],(uint32_t)src0->ne[3], (uint32_t)src0->nb[0] / src0_type_size, (uint32_t)src0->nb[1] / src0_type_size, (uint32_t)src0->nb[2] / src0_type_size, (uint32_t)src0->nb[3] / src0_type_size,
        (uint32_t)src1->ne[0], (uint32_t)src1->ne[1], (uint32_t)src1->ne[2],(uint32_t)src1->ne[3], (uint32_t)src1->nb[0] / src1_type_size, (uint32_t)src1->nb[1] / src1_type_size, (uint32_t)src1->nb[2] / src1_type_size, (uint32_t)src1->nb[3] / src1_type_size,
        (uint32_t) dst->ne[0], (uint32_t) dst->ne[1], (uint32_t) dst->ne[2],(uint32_t) dst->ne[3], (uint32_t) dst->nb[0] /  dst_type_size, (uint32_t) dst->nb[1] /  dst_type_size, (uint32_t) dst->nb[2] /  dst_type_size, (uint32_t) dst->nb[3] /  dst_type_size,
...
GGML_CALL int ggml_backend_vk_reg_devices() {
    ...
    for (size_t i = 0; i < vk_instance.device_indices.size(); i++) {
        char name[128];
        snprintf(name, sizeof(name), "%s%ld", GGML_VK_NAME, (long)i);
...
static void ggml_vk_check_results_0(ggml_tensor * tensor) {
    ...
    } else if (tensor->op == GGML_TYPE_F16) {
