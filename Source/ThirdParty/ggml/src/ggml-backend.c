...
static bool ggml_are_same_layout_backend(const struct ggml_tensor * a, const struct ggml_tensor * b) {
    if (a->type != b->type) {
        return false;
    }
...
void ggml_backend_tensor_copy(struct ggml_tensor * src, struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_are_same_layout_backend(src, dst) && "cannot copy tensors with different layouts");
...
GGML_CALL static ggml_backend_graph_plan_t ggml_backend_cpu_graph_plan_create(ggml_backend_t backend, const struct ggml_cgraph * cgraph) {
    ...
    struct ggml_backend_plan_cpu * cpu_plan = (struct ggml_backend_plan_cpu *)malloc(sizeof(struct ggml_backend_plan_cpu));
...
ggml_backend_t ggml_backend_cpu_init(void) {
    struct ggml_backend_cpu_context * ctx = (struct ggml_backend_cpu_context *)malloc(sizeof(struct ggml_backend_cpu_context));
    if (ctx == NULL) {
        return NULL;
    }
        ...
        for (int i = 0; i < graph->n_nodes; i++) {
            ...
            if (node_backend_id != cur_backend_id || need_new_split) {
                ...
                if (i_split >= sched->splits_capacity) {
                    ...
                    sched->splits = (struct ggml_backend_sched_split *)realloc(sched->splits, sched->splits_capacity * sizeof(struct ggml_backend_sched_split));
...
ggml_backend_sched_t ggml_backend_sched_new(
    ...
    struct ggml_backend_sched * sched = (struct ggml_backend_sched *)calloc(1, sizeof(struct ggml_backend_sched));

    sched->debug = getenv("GGML_SCHED_DEBUG") != NULL;

    // initialize hash table
    sched->hash_set          = ggml_hash_set_new(graph_size);
    sched->tensor_backend_id = (int *)calloc(sched->hash_set.size, sizeof(sched->tensor_backend_id[0]));
    sched->tensor_copies     = static_cast<ggml_tensor*(*)[GGML_SCHED_MAX_BACKENDS][GGML_SCHED_MAX_COPIES]>(calloc(sched->hash_set.size, sizeof(sched->tensor_copies[0])));
    ...
    sched->splits = (struct ggml_backend_sched_split *)calloc(initial_splits_capacity, sizeof(sched->splits[0]));
...
struct ggml_backend_graph_copy_struct ggml_backend_graph_copy(ggml_backend_t backend, struct ggml_cgraph * graph) {
    struct ggml_hash_set hash_set = {
        ...
        /* .keys = */ (struct ggml_tensor **)calloc(graph->visited_hash_table.size, sizeof(hash_set.keys[0])) // NOLINT
    };
    struct ggml_tensor ** node_copies = (struct ggml_tensor **)calloc(hash_set.size, sizeof(node_copies[0])); // NOLINT
...
bool ggml_backend_compare_graph_backend(ggml_backend_t backend1, ggml_backend_t backend2, struct ggml_cgraph * graph, ggml_backend_eval_callback callback, void * user_data) {
    ...
    for (int i = 0; i < g1->n_nodes; i++) {
        ...
        assert(t1->op == t2->op && ggml_are_same_layout_backend(t1, t2));
