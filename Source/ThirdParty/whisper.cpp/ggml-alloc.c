...
ggml_gallocr_t ggml_gallocr_new_n(ggml_backend_buffer_type_t * bufts, int n_bufs) {
    ...
    galloc->bufts = (ggml_backend_buffer_type_t*)calloc(n_bufs, sizeof(ggml_backend_buffer_type_t));
    GGML_ASSERT(galloc->bufts != NULL);

    galloc->buffers = (ggml_backend_buffer_t*)calloc(n_bufs, sizeof(ggml_backend_buffer_t) * n_bufs);
...
bool ggml_gallocr_reserve_n(ggml_gallocr_t galloc, struct ggml_cgraph * graph, const int * node_buffer_ids, const int * leaf_buffer_ids) {
    ...
    if (galloc->hash_set.size < hash_size) {
        ...
        galloc->hash_set.keys = (ggml_tensor **)calloc(hash_size, sizeof(struct ggml_tensor *));
        galloc->hash_values   = (hash_node *)calloc(hash_size, sizeof(struct hash_node));
