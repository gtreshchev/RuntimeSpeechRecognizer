        ...
        *obj_new = [&]()
        {
            ...
            obj.offs = cur_end + GGML_OBJECT_SIZE;
        ...
        *obj_new = [&]()
        {
            struct ggml_object obj;
            obj.offs = cur_end + GGML_OBJECT_SIZE;
            obj.size = sizeof(struct ggml_tensor);
            obj.next = NULL;
            return obj;
        }();
        // RuntimeSpeechRecognizer changes
...
static void ggml_compute_forward_mul_mat_f32(
    ...
    if (params->type == GGML_TASK_FINALIZE) {
        ...
        float * const wdata = (float * const)params->wdata;
        // RuntimeSpeechRecognizer changes
...
static void ggml_compute_forward_mul_mat_f16_f32(
    ...
    if (params->type == GGML_TASK_INIT) {
        if (nb01 >= nb00) {
            ...
            ggml_fp16_t * const wdata = (ggml_fp16_t * const)params->wdata;
            // RuntimeSpeechRecognizer changes
    ...
    if (nb01 >= nb00) {
        ...
        ggml_fp16_t * wdata = (ggml_fp16_t * const)params->wdata;
        // RuntimeSpeechRecognizer changes
    ...
    if (n_threads > 1) {
        ...
        for (int j = 0; j < n_threads - 1; j++) {
            ...
            workers[j] = [&]()
            {
                ...
                worker.thrd = 0;
                {
                    worker.params.type = GGML_TASK_COMPUTE;
                    worker.params.ith = j + 1;
                    worker.params.nth = n_threads;
                    worker.params.wsize = cgraph->work ? ggml_nbytes(cgraph->work) : 0;
                    worker.params.wdata = cgraph->work ? cgraph->work->data : NULL;
                }
                worker.node = NULL;
                worker.shared = &state_shared;
                return worker;
            }();
            // RuntimeSpeechRecognizer changes
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (node->n_tasks > 1) {
            ...
            for (int j = 0; j < n_threads - 1; j++) {
                ...
                workers[j].params = [&]()
                {
                    struct ggml_compute_params worker_params;
                    worker_params.type = GGML_TASK_COMPUTE;
                    worker_params.ith = j + 1;
                    worker_params.nth = node->n_tasks;
                    worker_params.wsize = cgraph->work ? ggml_nbytes(cgraph->work) : 0;
                    worker_params.wdata = cgraph->work ? cgraph->work->data : NULL;
        ...
        if (node->n_tasks > 1) {
            ...
            for (int j = 0; j < n_threads - 1; j++) {
                ...
                {
                    struct ggml_compute_params worker_params;
                    worker_params.type = GGML_TASK_FINALIZE;
                    worker_params.ith = j + 1;
                    worker_params.nth = node->n_tasks;
                    worker_params.wsize = cgraph->work ? ggml_nbytes(cgraph->work) : 0;
                    worker_params.wdata = cgraph->work ? cgraph->work->data : NULL;
...
static enum ggml_opt_result ggml_opt_adam(
    ...
    float * pf = params.past > 0 ? (float*)ggml_new_tensor_1d(ctx, GGML_TYPE_F32, params.past)->data : NULL; // past function values
    // RuntimeSpeechRecognizer changes
        ...
        case GGML_OPT_ADAM:
            {
                ...
                result = [&]()
                {
                    struct ggml_opt_params result_internal;
                    result_internal.type = GGML_OPT_ADAM;
                    result_internal.n_threads = 1;
                    result_internal.past = 0;
                    result_internal.delta = 1e-5f;
                    result_internal.max_no_improvement = 100;
                    result_internal.print_forward_graph = true;
                    result_internal.print_backward_graph = true;
                    {
                        result_internal.adam.n_iter = 10000;
                        result_internal.adam.alpha = 0.001f;
                        result_internal.adam.beta1 = 0.9f;
                        result_internal.adam.beta2 = 0.999f;
                        result_internal.adam.eps = 1e-8f;
                        result_internal.adam.eps_f = 1e-5f;
                        result_internal.adam.eps_g = 1e-3f;
                    }
        ...
        case GGML_OPT_LBFGS:
            {
                ...
                {
                    struct ggml_opt_params result_internal;
                    result_internal.type = GGML_OPT_LBFGS;
                    result_internal.n_threads = 1;
                    result_internal.past = 0;
                    result_internal.delta = 1e-5f;
                    result_internal.max_no_improvement = 0;
                    result_internal.print_forward_graph = true;
                    result_internal.print_backward_graph = true;
                    {
                        result_internal.lbfgs.m = 6;
                        result_internal.lbfgs.n_iter = 100;
                        result_internal.lbfgs.max_linesearch = 20;
                        result_internal.lbfgs.eps = 1e-5f;
                        result_internal.lbfgs.ftol = 1e-4f;
                        result_internal.lbfgs.wolfe = 0.9f;
                        result_internal.lbfgs.min_step = 1e-20f;
                        result_internal.lbfgs.max_step = 1e+20f;
                        result_internal.lbfgs.linesearch = GGML_LINESEARCH_DEFAULT;
                    }
                    return result_internal;
                }();
                // RuntimeSpeechRecognizer changes
