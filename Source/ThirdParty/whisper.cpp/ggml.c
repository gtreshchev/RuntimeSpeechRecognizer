...
static struct ggml_tensor * ggml_rope_impl(
    ...
    int32_t params[13] = { /*n_past*/ 0, (int32_t)n_dims, (int32_t)mode, (int32_t)n_ctx, (int32_t)n_orig_ctx };
...
struct ggml_tensor * ggml_im2col(
    ...
    int32_t params[] = { (int32_t)s0, (int32_t)s1, (int32_t)p0, (int32_t)p1, (int32_t)d0, (int32_t)d1, (int32_t)(is_2D ? 1 : 0) };
    ggml_set_op_params(result, params, sizeof(params));
    ...
    for (int64_t i12 = 0; i12 < ne12; ++i12) {
        for (int64_t i11 = 0; i11 < ne11; ++i11) {
            for (int64_t i10 = 0; i10 < ne10; ++i10) {
                ...
                ggml_fp16_to_fp32_row((ggml_fp16_t*)
                        (const void *) ((char *) src0->data + i01*nb01 + i11*nb02 + i12*nb03),
                             (float *) ((char *)  dst->data + i10*nb1  + i11*nb2  + i12*nb3), nc);
            }
        }
...
static void ggml_compute_forward_soft_max_f32(
    ...
    float * pos = (float * )(src2 ? (float *) src2->data : src0->data);
...
static void ggml_compute_forward_pool_2d(
    ...
    const int32_t * opts = (const int32_t *)dst->op_params;
    enum ggml_op_pool op = (enum ggml_op_pool)opts[0];
    const int k0 = opts[1];
    const int k1 = opts[2];
    const int s0 = opts[3];
    const int s1 = opts[4];
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        for (int irep = 0; irep < nrep; ++irep) {
            ...
            for (int iq1 = 0; iq1 < neq1; ++iq1) {
                ...
                ggml_vec_set_f32(masked_begin, (float*)S, 0);
...
enum ggml_status ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    struct ggml_compute_state_shared state_shared = {
        ...
        /*.n_threads               =*/ n_threads,
        /*.n_active                =*/ ATOMIC_BOOL_INIT(n_threads),
        /*.node_n                  =*/ ATOMIC_BOOL_INIT(-1),
        /*.node_task               =*/ ATOMIC_BOOL_INIT(GGML_TASK_TYPE_FINALIZE),
        /*.abort_callback          =*/ NULL,
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    ...
    // read the kv pairs
    {
        ctx->kv = (gguf_kv*)GGML_MALLOC(ctx->header.n_kv * sizeof(struct gguf_kv));
    ...
    {
        ctx->infos = (gguf_tensor_info*)GGML_MALLOC(ctx->header.n_tensors * sizeof(struct gguf_tensor_info));
            ...
            case GGUF_TYPE_ARRAY:
                {
                    if (src->kv[i].value.arr.type == GGUF_TYPE_STRING) {
                        const char ** data = (const char **)GGML_MALLOC(src->kv[i].value.arr.n*sizeof(char *));
